...
contract Cashier is ICashier, UsingHelpers, ReentrancyGuard, Ownable, Pausable {
        ...
        notZeroAddress(token)
    {
        ...
        emit LogWithdrawTokensOnDisaster(amount, token, msg.sender);
    }
        ...
        internal
    {
        if (
            ...
            escrow[voucherDetails.holder] = escrow[voucherDetails.holder] - voucherDetails.price;
        ...
        voucherDetails.price2issuer = voucherDetails.price2issuer + voucherDetails.price;
        ...
        internal
    {
        ...
        voucherDetails.price2holder = voucherDetails.price2holder + voucherDetails.price;
    ...
    function distributeIssuerDepositOnHolderComplain(
        ...
        if (isStatus(voucherDetails.currStatus.status, IDX_CANCEL_FAULT)) {
            ...
            if (
                ...
                escrow[voucherDetails.issuer] = escrow[voucherDetails.issuer] - voucherDetails.depositSe;
            }
            ...
            uint256 tFraction = voucherDetails.depositSe/CANCELFAULT_SPLIT;
            voucherDetails.deposit2holder = voucherDetails.deposit2holder + tFraction; //Bu gets, say, a half
            voucherDetails.deposit2issuer = voucherDetails.deposit2issuer + (tFraction/CANCELFAULT_SPLIT);//Se gets, say, a quarter
            voucherDetails.deposit2pool = voucherDetails.deposit2pool + ((voucherDetails.depositSe - tFraction) - tFraction/CANCELFAULT_SPLIT);//slashing the rest
            ...
            emit LogAmountDistribution(
                voucherDetails.tokenIdVoucher,
                voucherDetails.issuer,
                tFraction/CANCELFAULT_SPLIT,
                PaymentType.DEPOSIT_SELLER
            );
            emit LogAmountDistribution(
                voucherDetails.tokenIdVoucher,
                owner(),
                (voucherDetails.depositSe - tFraction) - (
                    tFraction/CANCELFAULT_SPLIT
                ),
                PaymentType.DEPOSIT_SELLER
            );
            ...
            voucherDetails.deposit2pool = voucherDetails.deposit2pool + voucherDetails.depositSe;
    ...
    function distributeIssuerDepositOnIssuerCancel(
        ...
        voucherDetails.deposit2issuer = voucherDetails.deposit2issuer + (
            voucherDetails.depositSe/CANCELFAULT_SPLIT
        );

        voucherDetails.deposit2holder = voucherDetails.deposit2holder + (
            voucherDetails.depositSe - (voucherDetails.depositSe/CANCELFAULT_SPLIT)
        );
        ...
        internal
    {
        ...
        voucherDetails.deposit2issuer = voucherDetails.deposit2issuer + voucherDetails.depositSe;
        ...
        internal
    {
        if (
            ...
            escrow[voucherDetails.holder] = escrow[voucherDetails.holder] - voucherDetails.depositBu;
        }
        ...
        voucherDetails.deposit2holder = voucherDetails.deposit2holder + voucherDetails.depositBu;
    ...
    function distributeHolderDepositOnNotRedeemedNotCancelled(
        ...
        voucherDetails.deposit2pool = voucherDetails.deposit2pool - voucherDetails.depositBu;
    ...
    function withdrawDepositsSe(
        ...
        uint256 depositAmount = deposit * _burnedQty;
        ...
        if (paymentMethod == ETHETH || paymentMethod == TKNETH) {
            escrow[_msgSender] = escrow[_msgSender] - depositAmount;
        }
    ...
    function addEscrowAmount(address _account)
        ...
        onlyFromRouter
    {
        escrow[_account] = escrow[_account] + msg.value;
    }
    ...
    function addEscrowTokensAmount(
        ...
        escrowTokens[_token][_account] =  escrowTokens[_token][_account] + _newAmount;
    }
    ...
    function onERC721Transfer(
        ...
        if (paymentType == ETHETH) {
            ...
            escrow[_from] = escrow[_from] - totalAmount;
            escrow[_to] = escrow[_to]+ totalAmount;
        }
        ...
        if (paymentType == ETHTKN) {
            ...
            escrow[_from] = escrow[_from] - price;
            escrow[_to] = escrow[_to] + price;
            ...
            escrowTokens[tokenAddress][_from] = escrowTokens[tokenAddress][_from] - depositBu;
            escrowTokens[tokenAddress][_to] = escrowTokens[tokenAddress][_to] + depositBu;
        ...
        if (paymentType == TKNETH) {
            ...
            escrowTokens[tokenAddress][_from] = escrowTokens[tokenAddress][_from] - price;
            escrowTokens[tokenAddress][_to] = escrowTokens[tokenAddress][_to] + price;

            //Reduce _from escrow amount and increase _to escrow amount - deposit
            escrow[_from] = escrow[_from] - depositBu;
            escrow[_to] = escrow[_to] + depositBu;
        }
    ...
    function onERC1155Transfer(
        ...
        if (paymentType == ETHTKN || paymentType == TKNTKN) {
            ...
            //Reduce _from escrow token amount and increase _to escrow token amount - deposit
            escrowTokens[tokenDepositAddress][_from] = escrowTokens[tokenDepositAddress][_from] - totalAmount;
            escrowTokens[tokenDepositAddress][_to] = escrowTokens[tokenDepositAddress][_to] + totalAmount;
        }
