...
module.exports = {
   ...
   * the defaults web3 uses to send transactions. If you don't specify one
   * truffle will spin up a development blockchain for you on port 9545 when
   * you run `develop` or `test`. You can ask a truffle command to use a
   * specific network from the command line, e.g
   *
  ...
  networks: {
    ...
    // it by default if it's defined here and no other network is specified
    // at the command line.
    //
    // You should run a client (like ganache-cli, geth or parity) in a
    // separate terminal tab if you use this network and you must also set
    // the `host`, `port` and `network_id`
